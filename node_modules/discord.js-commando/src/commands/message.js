...
class CommandMessage {
	...
	async run() { // eslint-disable-line complexity
		...
		if(!hasPermission || typeof hasPermission === 'string') {
			...
			else return this.reply(`You do not have permission to use the \`${this.command.name}\` command.`).then(replyObj => replyObj.delete(10000));
		}
		...
		if(throttle && throttle.usages + 1 > this.command.throttling.usages) {
			...
			).then(replyObj => replyObj.delete(10000));
		}
		...
		if(!args && this.command.argsCollector) {
			...
			if(result.cancelled) {
				...
				return this.reply('Cancelled command.').then(replyObj => replyObj.delete(10000));
			}
			args = result.values;
		}
