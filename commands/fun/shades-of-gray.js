...
class ShadesOfGrayCommand extends commando.Command {
    constructor(client) {
        super(client, {
            name: "shadesofgray",
            group: "fun",
            memberName: "shadesofgray",
            description: "Autism Simulator.",
            examples: [ "shadesofgray 3" ],
            args: [
                {
                    key: 'lines',
    ...
    async run(message, {lines}) { 
        ...
        function parseStr(sentence) {
            var output = "";
            var openPos, closePos, token, parts, items;
            
            while (sentence.length > 0) {
                openPos = sentence.indexOf("[")

                if (openPos > -1) {
                    output = output + sentence.substring(0, openPos)
                    sentence = sentence.substring(openPos + 1)
                    closePos = sentence.indexOf("]")
                    token = sentence.substring(0, closePos)
                    parts = token.split("-")
                    ...
                    sentence = sentence.substring(closePos + 1)
                } else {
        ...
        while (counter > 0) {
            ...
            let temp = sentences[counter]
            sentences[counter] = sentences[index]
            sentences[index] = temp
        }

        for (var i = 0; i < parseInt(lines); i++) {
            str = str + parseStr(sentences[i]) + " "
        }
        
        var edgyEmbed = new RichEmbed()
            .setTitle("50 Shades Of WTF")
            .setDescription(str)
            .setColor(0xFFFFFF)

        message.channel.send({embed: edgyEmbed})
            .then(message.delete());
    }
}
